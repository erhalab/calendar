...
interface SelectTaskNotifyTimesPopupCallback {
    fun onFinished(notifyTimesString: String)
}

class SelectTaskNotifyTimesPopup(@NonNull context: Context?) : CenterPopupView(context!!),
    ...
    var callback: SelectTaskNotifyTimesPopupCallback? = null
    var selectedTimes: ArrayList<Int>? = null
    ...
    override fun onCreate() {
        ...
        selectedTimes?.apply {
            for (i in this) {
                ...
                if (!find) {
                    notifyTimeItems.add(NotifyTimeItem(minutes = i, select = true))
                }
            }
        }
        //对所有要显示的进行排序
        ...
        addMoreClickZone.setOnClickListener {
            AddNotifyTimePopup(context).apply {
                addNotifyTimeCallBack = this@SelectTaskNotifyTimesPopup
